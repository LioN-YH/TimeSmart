...
model_name=TimeSmart_moe_single
vlm_type=vilt
gpu=0
image_size=56
norm_const=0.4
...
percent=1
train_epochs=30
results_path=TimeSmart_moe_single.txt
results_folder=./Result_moe_single/

# Create Logs_moe_single directory if it doesn't exist
if [ ! -d "Logs_moe_single" ]; then
    mkdir Logs_moe_single
fi
...
run_experiment() {
    ...
    log_file="Logs_moe_single/${model_name}_${dset}_${seq_len}_${pred_len}_${percent}p.log"
