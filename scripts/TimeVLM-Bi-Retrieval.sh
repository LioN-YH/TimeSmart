...
model_name=TimeVLM-Bi-Retrieval
vlm_type=vilt
gpu=0
image_size=56
norm_const=0.4
...
percent=1
train_epochs=30
results_path=TimeVLM_Bi.txt
results_folder=./Result_Bi/

# Create Logs_Bi directory if it doesn't exist
if [ ! -d "Logs_Bi" ]; then
    mkdir Logs_Bi
fi
...
run_experiment() {
    ...
    log_file="Logs_Bi/${model_name}_${dset}_${seq_len}_${pred_len}_${percent}p.log"
