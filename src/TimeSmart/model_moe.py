...
class Model(nn.Module):
    """
    TimeSmart model with image and text modalities for enhanced time series forecasting.
    """
    ...
    def vision_augmented_learner(self, x_enc):
        ...
        # 2. 计算时序图像化类型权重
        ts2img_weights = self.router(meta_tensor)  # [B, d_ts2img]
        ...
        # 4. 加权融合时序图像化表示
        images = self._fuse_ts2img_representations(ts2img_tensor_list, ts2img_weights)
