...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc=None, x_dec=None, x_mark_dec=None, mask=None):
        ...
        # CHANGE: Image Standardization (move from VE.py)
        # Calculate mean/std per image
        img_mean = images.mean(dim=(2, 3, 4), keepdim=True).detach()
        img_std = images.std(dim=(2, 3, 4), keepdim=True).detach() + 1e-6
        ...
        # CHANGE: Inverse Standardization on Embeddings
        img_mean_flat = img_mean.view(B * D, 1)
        img_std_flat = img_std.view(B * D, 1)
        vision_embeddings = vision_embeddings * img_std_flat + img_mean_flat
