...
"""
    data_loader_save.py在data_loader.py基础上修改，在__getitem__中新增index作为返回值
    用于关联原始数据和预计算文件
"""
...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        # 1. 使用 pandas 读取 CSV 文件，得到原始 DataFrame
        df_raw = pd.read_csv(os.path.join(self.root_path, self.data_path))

        # 2. 划分训练/验证/测试集（3:1:1）边界
        border1s = [
            0,
        ...
        # scale用于控制是否启用标准化
        if self.scale:
            # 提取训练集的统计信息来标准化验证集/测试集，避免数据泄露
            # .fit() 方法会根据传入的数据（这里是训练集）计算每个特征的均值和标准差
            # .transform() 方法会根据之前 .fit() 得到的均值和标准差，对输入数据进行标准化
        ...
        # 使用 pandas 的 to_datetime() 函数将字符串格式的日期（如 '2016-07-01 00:00:00'）转换为 datetime 类型，以便后续提取时间特征
        df_stamp["date"] = pd.to_datetime(df_stamp.date)
        # self.timeenc == 0，手动提取基本时间特征
        if self.timeenc == 0:
            # lambda row: row.month 表示提取月份
            # 1 表示按行操作
        ...
        # self.timeenc == 1，使用高级时间编码函数time_features
        elif self.timeenc == 1:
        ...
        # 6. 保存数据
        # data_x 和 data_y 是模型输入输出数据
        self.data_x = data[border1:border2]
        self.data_y = data[border1:border2]

        # 如果是训练集且启用了增强 (augmentation_ratio > 0)，则进行数据增强
    ...
    # 获取一个样本
    # seq_x: 输入的历史序列（长度为 seq_len）
    # seq_y: 输出的目标序列（包括已知部分 label_len 和预测部分 pred_len）
    # seq_x_mark: 输入的时间戳特征
    # seq_y_mark: 输出的时间戳特征
    ...
    # 返回总样本数
    def __len__(self):
        # 计算可以切片出多少个完整的样本（根据 seq_len 和 pred_len）
        return len(self.data_x) - self.seq_len - self.pred_len + 1

    # 反标准化：用于将模型输出的标准化结果还原回原始尺度（便于可视化或评估）
